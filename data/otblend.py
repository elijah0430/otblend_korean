...
class OTblend:
    def __init__(self, word1, word2, head=0):
        ...
        self.word1 = word1
        self.word2 = word2
        self.word1_decomposed = self.processor.decompose(word1)
        self.word2_decomposed = self.processor.decompose(word2)
        self.head = head
    ...
    def __str__(self):
        return f"OTblend with words: '{self.word1}' and '{self.word2}'"

    def generate_candidates(self, head):
        if head == 0:
            word1 = self.word2_decomposed
            word2 = self.word1_decomposed
            word11 = self.word2
            word22 = self.word1
        else:
            word1 = self.word1_decomposed
            word2 = self.word2_decomposed
            word11 = self.word1
            word22 = self.word2

        word2_ls = [word2[i:] for i in range(0, len(word2) - len(self.processor.decompose(word22[-1])) + 1)]
        word1_ls = [word1[:i] for i in range(2, len(word1) + 1)]
        candidate_dict = {}

        for w1 in word1_ls:
            for w2 in word2_ls:
                candidate_dict[w1 + w2] = len(w1) - 1
        candidate_ls = candidate_dict.keys()
        candidate_ls = [word for word in candidate_ls if self.processor.check_word(word)]
        candidate_ls = [word for word in candidate_ls if len(self.processor.compose(word)) > 1]
    ...
    def apply_disc(self, candidate_score_dic, index):
        ...
        if len(self.word1) >= len(self.word2):
            longer_word = self.word1_decomposed
            shorter_word = self.word2_decomposed
        else:
            longer_word = self.word2_decomposed
            shorter_word = self.word1_decomposed
        sublen = len(self.processor.compose(longer_word)) - len(self.processor.compose(shorter_word))
        sublen_decomposed = len(self.processor.decompose(self.processor.compose(longer_word)[:sublen]))
            ...
            for i, c in enumerate(candidate):
                if i < len(longer_word):
                    if c == longer_word[i] and (
                        ...
                        (i >= sublen_decomposed and (i - sublen_decomposed) < len(shorter_word) and c == shorter_word[i - sublen_decomposed])
                    ):
                        switch = False
                        break
            if switch:
    ...
    def apply_max_dep(self, candidate_score_dic, index, head):
        if head < 2:
            ...
            shorter_word = self.word1
        for candidate in list(candidate_score_dic.keys()):
            dep = len(self.processor.compose(candidate)) - len(longer_word)
            max1 = len(longer_word) - len(self.processor.compose(candidate))
            if dep > 0:
    ...
    def apply_max_seg(self, candidate_score_dic, index):
        for candidate in list(candidate_score_dic.keys()):
            for i, char in enumerate(self.word1_decomposed):
                if i < len(candidate):
                    if char != candidate[i]:
                        candidate_score_dic[candidate][index] += len(self.word1_decomposed) - i
                        break
            for j, char in enumerate(self.word2_decomposed):
                if self.word2_decomposed[j:] == candidate[i:]:
                    break
                candidate_score_dic[candidate][index] += 1
        return candidate_score_dic
    ...
    def apply_segmental_conservation(self, candidate_score_dic, index):
        for candidate in list(candidate_score_dic.keys()):
            candidate_composed = self.processor.compose(candidate)
            for seg in candidate_composed:
    ...
    def order_candidates(self, length=5):
        ...
        composed_top_candidates = [(self.processor.compose(candidate), score) for candidate, score in top_candidates]
        return composed_top_candidates

    def score_candidate(self, candidate):
        sorted_candidates = sorted(self.candidate_score_dic.items(), key=lambda item: item[1])
        sorted_candidate_names = [item[0] for item in sorted_candidates]

        if self.processor.decompose(candidate) in sorted_candidate_names:
            return sorted_candidate_names.index(self.processor.decompose(candidate)) + 1
        else:
            return None  # 후보군에 없는 경우
